...
public class DiagnosticErrorRendererTest {
    ...
    /* testedClasses: ninja/diagnostics/DiagnosticErrorRenderer.java */
    /*
    ...
    @Test
    public void appendTabBeginInputNegativeOutputNotNull()
        throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {

      // Arrange
      final DiagnosticErrorRenderer objectUnderTest =
          (DiagnosticErrorRenderer)Reflector.getInstance("ninja.diagnostics.DiagnosticErrorRenderer");
      Reflector.setField(objectUnderTest, "s", new StringBuilder("$$$$$$$$$"));
      final int index = -1_000_000;

      // Act
      final Class<?> classUnderTest = Reflector.forName("ninja.diagnostics.DiagnosticErrorRenderer");
      final Method methodUnderTest =
          classUnderTest.getDeclaredMethod("appendTabBegin", Reflector.forName("int"));
      methodUnderTest.setAccessible(true);
      final DiagnosticErrorRenderer retval =
          (DiagnosticErrorRenderer)methodUnderTest.invoke(objectUnderTest, index);

      // Assert result
      Assert.assertNotNull(retval);
      Assert.assertNotNull(Reflector.getInstanceField(retval, "s"));
      Assert.assertEquals("$$$$$$$$$        <div id=\"tab-1000000\" class=\"tab\">\n",
                          Reflector.getInstanceField(retval, "s").toString());
    }
